...
public class ModelConfig implements Serializable
{
	...
	{
		SharedPreferences pres = context.getSharedPreferences(GlobleConfig.app_PerferenceName, android.content.Context
				.MODE_PRIVATE);
...
//		firstLinePattern =  StringUtils.replace(temp, "\\r", "\r");
	...
	public static void initFirstLine(SharedPreferences pres)
	{
		...
		Log.d(TAG,"提取firstLinePatter是:"+temp);
		temp =  StringUtils.replace(temp, "\r", "");
		temp =  StringUtils.replace(temp, "\\n", "\n");
		firstLinePattern =  StringUtils.replace(temp, "\\r", "\r");
		...
		for(int i=0;i<firstLineReplaceArray.length;i++){
			Log.d(TAG,String.valueOf(firstLineReplaceArray[i]));
		}
	}
