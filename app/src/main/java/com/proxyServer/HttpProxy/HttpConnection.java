...
public class HttpConnection
{
	...
	public void setNewClient(Socket client) throws IOException
	{
		...
		this.clientIn = new BufferedInputStream(client.getInputStream());//进来的client
	...
	public void setNewServer(Socket server) throws IOException
	{
		...
		this.serverOut = new BufferedOutputStream(server.getOutputStream());//这才是真正发出请求stream

		serverSocket.setSoTimeout(AppConfig.timeout);
	}

	public BufferedInputStream getClientIN()//表示客户端进来的数据
	{
		return clientIn;
	}

	public BufferedInputStream getServerIN()//表示服务器进来的数据
	{
		return serverIn;
	}

	public BufferedOutputStream getClientOUT()//表示给客户端数据
	{
		return clientOut;
	}

	public BufferedOutputStream getServerOUT()//表示发给服务器的数据
	{
		return serverOut;
	}
